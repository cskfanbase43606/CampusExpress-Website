...
function Booking() {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log('Form Data:', formObject); // Log the entire form object

    // Disable the submit button to prevent multiple submissions
    event.target.querySelector('button[type="submit"]').setAttribute('disabled', true);

    try {
        const response = await fetch('https://script.google.com/macros/s/AKfycby3Es9FirZz5Cg38m9lZ9kT_vMDWCfXEt6Jfv2YXSCpdWai6PIgg-Bpk6peXsYz7E457A/exec', {
            method: 'POST',
    ...
    <div className="login-root">
      <form className="box-root flex-flex flex-direction--column center-center" style={{ gridArea: 'top / start / 8 / end' }} onSubmit={handleSubmit}>
        <div className="big-box">
          ...
          <input type="date" id="pickupDate" name="pickupDate" required min={minDate}/>
        </div>
            <div className="row2">
